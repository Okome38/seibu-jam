document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadPosts() {
        ...
        posts.forEach(post => {
            ...
            div.className = 'post';
            div.innerHTML = `<strong>${post.name || '匿名'}</strong> <em>${new Date(post.postedAt).toLocaleString()}</em><p>${post.message}</p>`;
...
fetch(url)
  ...
  .then(function (weather) {
    ...
    // 取得した現在の天気データ（インデックス0番目）を使う
    const currentWeatherData = {
      date: timeDefinesList[0],
      telop: weatherCode[weatherCodeList[0]][2],
      tempMin: tempsMinList[0],
      tempMax: tempsMaxList[0]
    };

    // 表示用テキスト
    const currentWeatherText = 
      `現在の天気 (${currentWeatherData.date}): ${currentWeatherData.telop} ` +
      `最低気温: ${currentWeatherData.tempMin}℃, 最高気温: ${currentWeatherData.tempMax}℃`;

    // 画面に表示
    document.getElementById("currentWeatherText").textContent = currentWeatherText;
    ...
    if (savedWeather) {
      console.log("保存された天気データ:", savedWeather);
    }

    // fetchの最後に追加してください
    const weatherText = savedWeather.telop;  
    const currentDate = new Date(currentWeatherData.date);  // currentWeatherData.dateからDateオブジェクトを作成
    const dayOfWeek = currentDate.getDay();  // 日曜日: 0, 月曜日: 1, ..., 土曜日: 6
    ...
    //天気に雨が入ってたら１７～１９時の混雑度を17時側に20％ずつシフト　金土日だったら全体を0.2倍
    if (dayOfWeek === 5 || dayOfWeek === 6 || dayOfWeek === 0) {  // 金曜日、土曜日、日曜日
      adjustedCongestion = adjustedCongestion.map(item => ({
      time: item.time,
      percent: item.percent * 0.2
      }));
    }else{
